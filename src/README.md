...
| `%close`             | `handleClose`        | Closes signups and opens gamemode voting (`voteOpen = true`) for joined players                                                          || `%vote`              | `handleVote`         | Casts/changes a player's gamemode vote; bare `%vote` shows tallies; options filtered by lobby size (no 3v3 with 4 players); nudges host when everyone voted |
| `%unvote`            | `handleUnvote`       | Withdraws a player's gamemode vote                                                                                                                   |
| `%endvote`           | `handleEndVote`      | Closes voting, applies the winning mode (or reports a tie for the host to break)                                                         |
