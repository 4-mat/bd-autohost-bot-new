...
type EffectCtx = {
  ...
  /** Pre-resolved target tile (from a "tile" prompt) for tile effects. */
...
function* handleConditional(
  { game, user, target, ability, messages, routeBuffsToUser, tilePos }: EffectCtx,
  effect: ConditionalEffect,
) {
  ...
  if (outcome === "then" || outcome === "unknown") {
    const thenMsgs = yield* applyEffectStream(game, user, target, effect.thenEffects, ability, routeBuffsToUser, tilePos);
    messages.push(...thenMsgs.map((m) => `    ${m}`));
  }
  if (outcome === "else" && effect.elseEffects) {
    const elseMsgs = yield* applyEffectStream(game, user, target, effect.elseEffects, ability, routeBuffsToUser, tilePos);
...
function* handleThirst(
  ...
  const thirstMsgs = yield* applyEffectStream(game, user, target, effect.effects, ability, routeBuffsToUser, tilePos);
