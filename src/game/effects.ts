...
function parseClause(clause: string): Effect[] {
  ...
  // Tile placement: "Place [up to N] Terrain tile[s]", "Wormhole", "Create X"
  const tilePlacement = parseTilePlacement(lower);
  if (tilePlacement.length > 0) return tilePlacement;
...
function parseTilePlacement(lower: string): Effect[] {
  // "Place [up to N] Terrain tile[s]"
  const placeMatch = lower.match(
    /^place(?:\s+up\s+to)?\s*(?:(\d+)\s+)?(.+?)(?:\s+tiles?)?$/,
  );
  if (placeMatch) {
    const terrain = placeMatch[2].trim();
    return [
      {
        type: "tile",
        terrain: capitalize(terrain),
        count: placeMatch[1] ? parseInt(placeMatch[1]) : 1,
      },
    ];
  }

  // "Wormhole" — portal, maps to Stone terrain
  if (lower === "wormhole") {
    return [{ type: "tile", terrain: "Stone", count: 1 }];
  }

  // "Create X" — structures like Silk Cocoon, maps to Stone
  const createMatch = lower.match(/^create\s+(.+)$/);
  if (createMatch) {
    return [{ type: "tile", terrain: "Stone", count: 1 }];
  }
...
export function applyEffects(
  ...
  for (const effect of effects) {
    switch (effect.type) {
      ...
      case "tile": {
        // Tile effects are applied by resolve.ts's applyTileEffects which
        // has access to the tile position. Here we just log a placeholder.
        messages.push(
          `  [Tile: ${effect.terrain}${effect.count > 1 ? ` x${effect.count}` : ""} — placed via tile-targeting flow]`,
        );
        break;
      }
