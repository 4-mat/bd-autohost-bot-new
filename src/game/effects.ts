...
function* handleStatMod(
  ...
  const recipient =
    effect.subject === "self" || (routeBuffsToUser && effect.type === "buff")
      ? user
      : target;
  ...
  if (effect.percent) {
    ...
    const amount = mult * Math.abs(effect.amount);
    recipient.buffs.push({
      stat: effect.stat,
      amount,
      rounds: effect.rounds ?? 1,
    ...
    messages.push(
      `  ${recipient.num} ${effect.type === "buff" ? "gains +" : "loses "}${Math.abs(effect.amount)} ${effect.stat.toUpperCase()}${effect.rounds ? `/${effect.rounds}` : ""}.`,
    );
  }
}
