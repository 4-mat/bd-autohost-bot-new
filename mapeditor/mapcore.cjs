...
(function (root, factory) {
	...
	const DEFAULT_TOKEN_COLORS = {
		P1: '#00A000', P2: '#0000C0', P3: '#B00000', P4: '#8B00C0',
		P5: '#E07B00', P6: '#555555', P7: '#0070A0', P8: '#A04000'
	};

	const NAMED_COLORS = {
		black: '#000000', white: '#FFFFFF', green: '#008000', blue: '#0000FF',
		red: '#FF0000', purple: '#800080', orange: '#FFA500', yellow: '#FFFF00',
	...
	function sanitizeName(name) {
		name = String(name || 'untitled').trim().toLowerCase()
			.replace(/[^a-z0-9_-]+/g, '-')
			.replace(/^-+|-+$/g, '')
			.slice(0, 40);
	...
	function normalizeColor(c) {
		...
		if (m) {
			return '#' + m[1].split('').map((ch) => ch + ch).join('').toLowerCase();
		}
	...
	function emptyMap(rows, cols, name) {
		...
		return { name: sanitizeName(name), displayName: displayFromName(sanitizeName(name)), rows, cols, tiles, tokens: {} };
	}
	...
	function normalizeMap(map) {
		...
		if (map.tokens && typeof map.tokens === 'object') {
			for (const name in map.tokens) {
				...
				const r = Math.floor(tok.row), c = Math.floor(tok.col);
				if (isNaN(r) || isNaN(c) || r < 0 || r >= rows || c < 0 || c >= cols) continue;
				tokens[name] = {
					row: r,
					col: c,
	...
	function parseHTML(html) {
		...
		for (const raw of rowsRaw) {
			...
			const isHeader = cells.some((c) => /<b[^>]*>\s*\d+\s*<\/b>/i.test(c.inner));
			if (isHeader) {
				cols = cells.length - 1;
				continue;
			}
		...
		return normalizeMap({ name: 'imported', rows: tiles.length, cols, tiles, tokens });
	}
	...
	function parseTxt(text, file) {
			...
			if (head) {
				...
				if (head[1] === 'name') {
					...
					if (!/^[a-z0-9_-]{1,40}$/.test(name)) {
						throw new Error(file + ':' + n + ': bad map name "' + name + '" — use lowercase letters, digits, - or _ (up to 40 chars, no spaces)');
					}
					if (/^gen\d*$/.test(name)) {
		...
		const tiles = rows.map(({ text, line }) => {
			if (text.length !== cols) {
				throw new Error(file + ':' + line + ': row has ' + text.length + ' cells, expected ' + cols + ' — every row must be the same length');
			}
			return text.split('').map((code) => {
				const t = CODE_TO_TERRAIN[code];
				if (t === undefined) {
					throw new Error(file + ':' + line + ': unknown terrain code "' + code + '" — see maps/README.md for the legend');
				}
				return t;
			});
		});
	...
	function tokenAt(map, r, c) {
		for (const name in map.tokens) {
			...
			if (t.row === r && t.col === c) return { name, color: t.color || tokenColorFor(name) };
		}
		return null;
	}
	...
	function toHTML(map) {
		...
		out += '<tr style="min-height:28px;height:24px"><td style="min-width:22px"></td>';
		for (let c = 0; c < cols; c++) out += '<td style="min-width:22px" align="center"><b>' + colLabel(c) + '</b></td>';
		out += '</tr>';
		for (let r = 0; r < rows; r++) {
			out += '<tr style="min-height:24px;height:24px"><td align="center"><b>' + rowLabel(r) + '</b></td>';
			for (let c = 0; c < cols; c++) {
				const terrain = (tiles[r] && TERRAINS[tiles[r][c]]) ? tiles[r][c] : 'normal';
