...
(function (root, factory) {
	...
	let GAMEMODE_MIN_SIZE = { ffa: 7, ntr: 5, jugg: 7, pvp: 7, '1v1': 7 };
	let MODE_ALIASES_OVERRIDE = null;

	// Replace the hard-coded alias/min-size tables with data generated from
	// src/data/gamemodes.ts (via seed.cjs → maps/modes.json). Called by the
	// editor after fetching modes.json; a no-op when not supplied.
	function setModeData(data) {
		if (!data) return;
		if (data.aliases && typeof data.aliases === 'object') MODE_ALIASES_OVERRIDE = data.aliases;
		if (data.minSizes && typeof data.minSizes === 'object') GAMEMODE_MIN_SIZE = data.minSizes;
	}
	...
	// By default these match the bot; the editor can call setModeData() with
	// maps/modes.json (generated by seed.cjs) to stay in lockstep.
	const MODE_ALIASES = {
	...
	function modeIdFor(mode) {
		...
		const table = MODE_ALIASES_OVERRIDE || MODE_ALIASES;
	...
	function translateBlock(map, sel, dr, dc) {
		...
		// Paste at the destination. Displace any token from OUTSIDE the moved
		// block that happens to sit on a destination cell (the moved token
		// wins), but never the block's own token that shifted with it.

		// First, place the moved block's tokens so a source token that landed
		// on its own prior destination isn't treated as a foreign occupant.
		...
		// Then displace any remaining token (from outside the block) sitting
		// on a destination cell.
		for (const bt of blockTokens) {
			const tr = nr0 + bt.row, tc = nc0 + bt.col;
			for (const name in map.tokens) {
				if (name === bt.name) continue;
				const o = map.tokens[name];
				if (o.row === tr && o.col === tc) delete map.tokens[name];
			}
		}

		// Paste the tiles at the destination.
		let i = 0;
		for (let r = s.r0; r <= s.r1; r++)
			for (let c = s.c0; c <= s.c1; c++)
