...
| `%vote [mode]`                        | Cast/change your gamemode vote (FFA, 1v1, 2v2, 3v3, 4v4, 2v2v2, NTR, Juggernaut, 2vJ/3vJ/4vJ, PvPJ, PvP NTR) while voting is open; options filter by lobby size; bare `%vote` shows tallies. Use `%wt modes` or `%wt <mode>` to learn what each mode is. |
